...
| 功能模块 | 需求命令 | 当前状态 | 后续填写位置 |
| :--- | :--- | :--- | :--- |
| 数据库管理 | `CREATE DATABASE`, `DROP DATABASE`, `SHOW DATABASES`, `USE` | 已接入 SQL-only 入口，基础文件创建/删除已实现 | `modules.database.*` |
| 表管理 | `CREATE TABLE`, `DROP TABLE`, `SHOW TABLES`, `DESC` | 已接入，基础 `.tb/.tdf/.tic/.tid/.trd` 文件创建已实现 | `modules.table.*` |
| 字段管理 | `ALTER TABLE ADD/MODIFY/DROP COLUMN` | 已接入，当前主要更新 `.tdf`，历史记录迁移待完善 | `modules.table.infrastructure.NativeTableGatewayImpl` |
| 数据管理 | `INSERT`, `SELECT`, `UPDATE`, `DELETE` | 已接入，基础 `.trd` 读写已实现；复杂表达式、约束、索引同步待完善 | `modules.record.*` |
| 索引管理 | `CREATE INDEX`, `DROP INDEX`, `SHOW INDEXES`, `REBUILD INDEX` | 已创建接口、应用服务、占位实现和 SQL 路由 | `modules.index.infrastructure.TodoIndexGatewayImpl` |
| 客户端管理 | `CONNECT`, `DISCONNECT`, `SHOW CLIENTS` | 已创建接口、应用服务、内存占位实现和 SQL 路由 | `modules.client.infrastructure.InMemoryClientSessionRegistry` |
| 事务管理 | `BEGIN`, `COMMIT`, `ROLLBACK` | 已创建接口、应用服务、内存占位实现和 SQL 路由 | `modules.transaction.infrastructure.InMemoryTransactionManagerPort` |
| 完整性管理 | `ALTER TABLE ADD/DROP CONSTRAINT`, `CHECK CONSTRAINTS`, `SHOW CONSTRAINTS` | 已创建接口、应用服务、占位实现和 SQL 路由 | `modules.integrity.infrastructure.TodoIntegrityGatewayImpl` |
| 数据库维护 | `BACKUP DATABASE`, `RESTORE DATABASE`, `SHOW BACKUPS`, `DELETE BACKUP` | 已创建接口、应用服务、占位实现和 SQL 路由 | `modules.maintenance.infrastructure.TodoMaintenanceGatewayImpl` |
| 安全性管理 | `CREATE USER`, `DROP USER`, `ALTER USER`, `GRANT`, `REVOKE`, `SHOW GRANTS` | 已创建接口、应用服务、内存用户占位实现和 SQL 路由 | `modules.security.infrastructure.InMemorySecurityGateway` |
...
前端所有页面函数都在 `frontend/src/api/dbms.js` 内转换为上述格式，不再调用 `/api/databases`、`/api/auth/login` 等 REST 路径。

### 12.3 创建表详细流转

1. `TableManage.vue` 收集表名、字段、类型和约束。
2. 页面调用 `createTable(db, data)`。
3. `frontend/src/api/dbms.js` 生成 SQL：

   ```sql
   CREATE TABLE USERS (ID INT PRIMARY KEY NOT NULL, NAME VARCHAR(50) NOT NULL)
   ```

4. 前端发送：

   ```js
   http.post('/sql/execute', { databaseName: db, sql })
   ```

5. `SqlController` 接收请求并交给 `SqlApplicationService`。
6. `SqlApplicationService` 去掉尾部分号、校验 SQL 白名单、调用 `SqlParser`。
7. `SqlParser` 解析为 `SqlCommand.CreateTable(tableName, columns)`。
8. `SqlExecutor` 路由到 `TableApplicationService#createTable`。
9. `TableApplicationService` 校验数据库名和表名，再调用 `TableGateway#createTable`。
10. `NativeTableGatewayImpl` 写入 `[数据库名].tb`，并创建 `.tdf`、`.tic`、`.trd`、`.tid` 等文件。
11. `SqlExecutor` 组装 message payload，`SqlController` 用 `ApiResponse` 返回前端。
12. 前端看到 `refreshTree=true` 后刷新数据库树和表列表。

### 12.4 查询记录详细流转

1. `RecordManage.vue` 调用：

   ```js
   queryRecords(db, table, { page: 1, size: 50, filters: { ID: 1 } })
   ```

2. `dbms.js` 生成：

   ```sql
   SELECT * FROM USERS WHERE ID = 1 LIMIT 0, 50
   ```

3. 后端解析为 `SqlCommand.Select`。
4. `SqlExecutor` 调用 `RecordApplicationService#query`。
5. `RecordApplicationService` 校验库名、表名并限制分页大小。
6. `NativeRecordGatewayImpl` 读取 `.tdf`，计算字段类型、长度和偏移量。
7. `NativeRecordGatewayImpl` 扫描 `.trd`，解析有效记录。
8. `SqlExecutor` 执行过滤、排序、LIMIT 和投影列整理。
9. 前端 API 层把 SQL table payload 转成页面分页结构：

   ```js
   { total, page, size, list }
   ```
...
```text
modules/index
├── application/IndexApplicationService.java
├── domain/IndexGateway.java
├── dto/IndexDefinitionRequest.java
└── infrastructure/TodoIndexGatewayImpl.java
```

前端调用 `createIndex()` 会生成：
...
-> IndexGateway
-> TodoIndexGatewayImpl
```

后续只需要填写 `TodoIndexGatewayImpl` 的方法体，不需要新增 Controller，也不需要改前端请求路径。其他未实现模块同理，只填写对应 `modules.<模块名>.infrastructure` 下的占位实现。

### 12.6 已统一的前端 API

`frontend/src/api/dbms.js` 保留语义函数，内部全部转换为 SQL-only 请求：

```js
listDatabases()
createDatabase({ name: 'TEST_DB' })
listTables('TEST_DB')
insertRecord('TEST_DB', 'USERS', { values: { ID: 1, NAME: 'Alice' } })
```
